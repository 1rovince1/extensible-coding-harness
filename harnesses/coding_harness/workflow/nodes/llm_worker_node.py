...
from ..prompts.pompt_utils import compile_prompt, load_prompt_template
...
class LLMWorkerNode:
    def __init__(self, tool_registry: dict, skill_registry: dict):
        self.tool_registry = tool_registry
        self.skill_registry = skill_registry


    async def setup_context(self, state):
        agent_type = state.get("agent_type", "generic_sub_agent")
        agent_skill_registry = self.skill_registry
        agent_tool_registry = self.tool_registry
        
        # prompt setting
        prompt_template = await load_prompt_template(prompt_file=f"{agent_type}.system")
        agent_skills_metadata = agent_skill_registry.SKILLS_METADATA
        ...
        return prompt, agent_tools, agent_tool_registry, agent_skill_registry


    @traceable
    async def __call__(self, state: MainAgentState | GenericSubAgentState):
        logger.info(f"Inside llm worker node, agent_type: {state.get("agent_type", None)}")
        logger.debug(f"state inside llm worker node: {state}")
        ...
        ) = await self.setup_context(state)
        ...
        logger.info("Exiting llm worker node")
        return {
            ...
            "skill_registry": agent_skill_registry.SKILL_REGISTRY if parsed_llm_response["tool_calls"] else {},
